 ...
 *
 * Generated on Wed Jun 19 12:55:59 2013,
 ...
 *****************************************************************************/
crc_t eics_crc_reflect(crc_t data, size_t data_len);
 ...
 *****************************************************************************/
static inline crc_t eics_crc_init(void)
{
 ...
 *****************************************************************************/
crc_t eics_crc_update(crc_t crc, const unsigned char *data, size_t data_len);
 ...
 *****************************************************************************/
static inline crc_t eics_crc_finalize(crc_t crc)
{
 ...
 *****************************************************************************/
static const crc_t eics_crc_table[256] = {
...
crc_t eics_crc_update(crc_t crc, const unsigned char *data, size_t data_len)
{
    register unsigned int tbl_idx;

    while (data_len--) {
        tbl_idx = (crc ^ *data) & 0xff;
        crc = (eics_crc_table[tbl_idx] ^ (crc >> 8)) & 0xffffffffffffffff;
...
uint64_t eics::eics_crc64(const unsigned char *data, size_t len) {
    register crc_t retval;
    retval = eics_crc_init();
    retval = eics_crc_update(retval, data, len);
    return eics_crc_finalize(retval);
}
